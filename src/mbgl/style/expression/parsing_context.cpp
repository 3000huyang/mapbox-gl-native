...
bool isConstant(const Expression& expression) {
    ...
    bool isTypeAnnotation = expression.getStrategy() == Strategy::Coercion ||
        expression.getStrategy() == Strategy::Assertion ||
        expression.getStrategy() == Strategy::ArrayAssertion;
    ...
    expression.eachChild([&](const Expression& child) {
        ...
        if (isTypeAnnotation) {
            ...
            childrenConstant = childrenConstant && child.getStrategy() == Strategy::Literal;
        }
    });
    if (!childrenConstant) {
        return false;
...
ParseResult ParsingContext::parse(const Convertible& value, TypeAnnotationOption typeAnnotationOption) {
    ...
    // parsed result.
    if ((*parsed)->getStrategy() != Strategy::Literal && isConstant(**parsed)) {
